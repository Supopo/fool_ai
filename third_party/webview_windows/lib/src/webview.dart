...
class WebviewController extends ValueNotifier<WebviewValue> {
  ...
  /// Inserts [text] into the focused page element via DevTools Input.insertText.
  ///
  /// Prefer this for rich editors (ProseMirror / Lexical) that ignore DOM
  /// value writes and leave the send button disabled.
  Future<void> insertText(String text) async {
    if (_isDisposed) {
      return;
    }
    assert(value.isInitialized);
    return _methodChannel.invokeMethod('insertText', text);
  }
            ...
            child: _controller.value.isInitialized
                ? Listener(
                    ...
                    onPointerDown: (ev) {
                      ...
                      // Sync hit-test position before the button event so a
                      // click after scrolling lands on the element under the
                      // cursor instead of a stale point.
                      _controller._setCursorPos(ev.localPosition);
