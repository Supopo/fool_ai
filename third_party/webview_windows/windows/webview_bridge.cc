...
constexpr auto kMethodInsertText = "insertText";
...
void WebviewBridge::HandleMethodCall(
  ...
  // insertText: string - Chrome DevTools Input.insertText (real editor input)
  if (method_name.compare(kMethodInsertText) == 0) {
    if (const auto text = std::get_if<std::string>(method_call.arguments())) {
      if (webview_->InsertText(*text)) {
        return result->Success();
      }
      return result->Error(kMethodFailed, "Input.insertText failed.");
    }
    return result->Error(kErrorInvalidArgs);
  }
