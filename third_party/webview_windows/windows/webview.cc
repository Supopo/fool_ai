...
Webview::Webview(
  ...
  // Keep WebView2's input HWND inside the Flutter view tree. With the default
  // message-only parent, the first click deactivates the app window, so text
  // selection needs a second click. See upstream issue #230.
  if (flutter_view_hwnd_) {
    webview_controller_->put_ParentWindow(flutter_view_hwnd_);
  }
...
void Webview::SendScroll(double delta, bool horizontal) {
  // SendMouseInput(WHEEL) often ignores any point other than (0, 0) in
  // composition mode. Dispatch through DevTools so the element under the
  // cursor (including nested chat panes) receives the wheel.
  const float scale = scale_factor_ > 0.f ? scale_factor_ : 1.f;
  const double css_x = static_cast<double>(last_cursor_pos_.x) / scale;
  const double css_y = static_cast<double>(last_cursor_pos_.y) / scale;
  // |delta| is WM-style (negated Flutter scrollDelta). CDP mouseWheel uses
  // DOM WheelEvent units: positive deltaY scrolls down.
  const double cdp_dx = horizontal ? -delta : 0.0;
  const double cdp_dy = horizontal ? 0.0 : -delta;
  const auto json = std::format(
      "{{\"type\":\"mouseWheel\",\"x\":{:.2f},\"y\":{:.2f},"
      "\"deltaX\":{:.2f},\"deltaY\":{:.2f}}}",
      css_x, css_y, cdp_dx, cdp_dy);
  if (SUCCEEDED(webview_->CallDevToolsProtocolMethod(
          L"Input.dispatchMouseEvent", util::Utf16FromUtf8(json).c_str(),
          nullptr))) {
    return;
  }
  ...
  composition_controller_->SendMouseInput(
      COREWEBVIEW2_MOUSE_EVENT_KIND_MOVE, virtual_keys_.state(), 0,
      last_cursor_pos_);
  POINT origin{0, 0};
  if (horizontal) {
    composition_controller_->SendMouseInput(
...
bool Webview::InsertText(const std::string& text) {
  if (!IsValid()) {
    return false;
  }

  // Build {"text":"..."} with JSON string escaping. UTF-8 payload is then
  // converted to UTF-16 for WebView2.
  std::string escaped;
  ...
  const auto json = std::format("{{\"text\":\"{}\"}}", escaped);
  return SUCCEEDED(webview_->CallDevToolsProtocolMethod(
      L"Input.insertText", util::Utf16FromUtf8(json).c_str(), nullptr));
}
